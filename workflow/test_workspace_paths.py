...
class TestWorkspacePaths(unittest.TestCase):
    ...
    def testCompositeSummary(self):
        ...
        # Observed data
        self.assertEqual(
            join(self.root, 'composite', 'composite_summary_201612.nc'),
            self.ws.composite_summary(yearmon='201612')
        )

        self.assertEqual(
            join(self.root, 'composite', 'composite_summary_36mo_201612.nc'),
            self.ws.composite_summary(yearmon='201612', window=36)
        )

        # Forecast data
        self.assertEqual(
            join(self.root, 'composite', 'composite_summary_201612_trgt201708.nc'),
            self.ws.composite_summary(yearmon='201612', target='201708')
        )

        self.assertEqual(
            join(self.root, 'composite', 'composite_summary_6mo_201612_trgt201708.nc'),
    ...
    def testReturnPeriodSummary(self):
        ...
        # Observed data
        self.assertEqual(
            join(self.root, 'summary', 'rp_summary_201612.nc'),
            self.ws.return_period_summary(yearmon='201612')
        )

        self.assertEqual(
            join(self.root, 'summary', 'rp_summary_36mo_201612.nc'),
            self.ws.return_period_summary(yearmon='201612', window=36)
        )

        # Forecast data
        self.assertEqual(
            join(self.root, 'summary', 'rp_summary_201612_trgt201708.nc'),
            self.ws.return_period_summary(yearmon='201612', target='201708')
        )

        self.assertEqual(
            join(self.root, 'summary', 'rp_summary_6mo_201612_trgt201708.nc'),
    ...
    def testResultsSummary(self):
        ...
        # Observed data
        self.assertEqual(
            join(self.root, 'summary', 'results_summary_201612.nc'),
            self.ws.results_summary(yearmon='201612')
        )

        self.assertEqual(
            join(self.root, 'summary', 'results_summary_36mo_201612.nc'),
            self.ws.results_summary(yearmon='201612', window=36)
        )

        # Forecast data
        self.assertEqual(
            join(self.root, 'summary', 'results_summary_201612_trgt201708.nc'),
            self.ws.results_summary(yearmon='201612', target='201708')
        )

        self.assertEqual(
            join(self.root, 'summary', 'results_summary_6mo_201612_trgt201708.nc'),
