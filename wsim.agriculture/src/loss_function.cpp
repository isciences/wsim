...
//' @export
// [[Rcpp::export]]
Rcpp::NumericVector loss_function(const Rcpp::NumericVector & rp) {
  ...
  constexpr double rp_onset = 6;
  constexpr double rp_total = 70;
  ...
  for(decltype(n) i = 0; i < n; i++) {
    if (rp[i] >= rp_total) {
      ...
      out[i] = 1 - std::pow((rp_total - rp[i])/(rp_total - rp_onset), 1.4);
    }
  }
